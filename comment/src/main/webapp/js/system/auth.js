// 初始化用户树 用户组树 菜单树对应后台的users、groud、menus、 并使用jq验证必填
$(function() {
	// 初始化用户树
	initUserTree();
	// 初始化用户组树
...
function move(element) {
	//相当于： <li class=rMenuLi onmousemove="move(this);">
...
function selectUser() {
	...
	common.ajax({
		...
		success : function(data) {
			// 如果当前选中的用户有用户组，则选中这个用户组（单击超级管理员根据sys_user的groud_id==1选中用户组的系统管理员）
			if(data.groupId) {
